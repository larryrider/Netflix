...
<head>
	...
	<link type="text/css" rel="stylesheet" href="/css/serie.css"/>
	<link rel="shortcut icon" href="/img/nficon2016.ico"/>
	<script type='text/javascript' src="https://code.jquery.com/jquery-3.2.1.min.js"></script>
</head>

<body>
<div id="modalResenyas" style="display: none;" data-reactroot="" class="nf-modal reviews-modal">
   <div class="nf-modal-background"></div>
   <div class="nf-modal-content" dir="ltr">
      <div class="nf-modal-content-wrapper">
            <div class="nf-modal-dismiss">
                <button id="btnCerrar" class="nf-modal-dismiss-btn" aria-label="close" tabindex="1">
                    <span class="close-button icon-close"></span>
                </button>
            </div>
         <div class="nf-modal-header">
            <div class="reviewsModalHeader">
                <h2 class="nf-modal-heading">{{$reviews->count()}}&nbsp;reseñas de los suscriptores para {{$serie->title}}</h2>

                <form url="/serie/{{$serie->id}}/details" method="POST" id="form">
                    {{ csrf_field() }}
                    <input type="hidden" id="_method" name="_method" value="POST"></input>
                    <input type="hidden" id="stars" name="stars"/>
                    <input type="hidden" id="serie" name="serie" value="{{$serie->id}}"/>
                    <input type="hidden" id="reviewId" name="reviewId"/>
                    <div id="newResenya" style="display: none;" class="writeReviewForm">
                        <div class="writeReviewSavingMask"><span class="icon-spinner akira-spinner" aria-hidden="true"></span></div>
                            <div class="rating">
                                Valora este título:
                                <div class="starbar" aria-hidden="true">
                                <span id="s1" class="star sb-placeholder icon-star" data-rating="1"></span>
                                <span id="s2" class="star sb-placeholder icon-star" data-rating="2"></span>
                                <span id="s3" class="star sb-placeholder icon-star" data-rating="3"></span>
                                <span id="s4" class="star sb-placeholder icon-star" data-rating="4"></span>
                                <span id="s5" class="star sb-placeholder icon-star" data-rating="5"></span>
                            </div>
                        </div>
                        <textarea id="reviewText" name="reviewText" class="reviewText" placeholder="Escribe tu reseña aquí."></textarea>
                        <div class="buttons">
                            <button type="submit" class="akira-button akira-button-red" >Enviar</button>
                            <button id="btnCancelar" class="akira-button">Cancelar</button>
                            <span class="spacer"></span>
                            <span class="faq">¿Tienes preguntas sobre las 
                                <a href="#">reseñas de los suscriptores?</a>
                            </span>
                        </div>
                    </div>
                </form>
               <button id="btnEscribir" class="akira-button akira-button-red">Escribir reseña</button>
            </div>
         </div>
         <hr class="nf-modal-hr">
         <div class="nf-modal-body">
            <div class="reviewsModalBody">
               <!-- react-empty: 15 -->
               <div>
                  <h3>Reseñas</h3>
                  <ul class="memberReviews">
                    @foreach($reviews as $r)
                        <li>
                            <div style="display:none;" id="review{{$r->id}}" stars="{{$r->stars}}" desc="{{$r->description}}"></div>
                            <div class="memberReview">
                            <div class="starbar readOnly" aria-hidden="true">
                                @for($i=1;$i<=$r->stars;$i++)
                                    <span class="star default icon-star"></span>
                                @endfor
                                @for($j=1;$j<=5-$r->stars; $j++)
                                <span class="star sb-placeholder icon-star" ></span>
                                @endfor
                            </div>
                            <div class="reviewText">{{$r->description}}</div>
                            <div class="helpfulCount"></div>
                            <div class="voteReview">
                                <span class="voteReviewPrompt"></span>
                                <div class="reviewActions">
                                    <ul class="voteReviewChoices">
                                        @if($r->user_id == Auth::id())
                                        <li class="akira-radio-item" id="botonesEditar">
                                            <input type="radio" class="akira-radio">
                                            <a id="{{$r->id}}" style="text-decoration: none;" class="akira-radio-label action">Editar</a>
                                        </li>
                                        <li class="akira-radio-item" id="botonesEliminar">
                                            <input type="radio" class="akira-radio">
                                            <form url="/serie/{{$serie->id}}/details" method="POST">
                                                {{method_field('DELETE')}}
                                                {{ csrf_field() }}
                                                <input type="hidden" id="id" name="id" value="{{$r->id}}"></input>
                                                <a onclick="$(this).closest('form').submit()" style="text-decoration: none;" class="akira-radio-label action">Eliminar</a>
                                            </form>
                                        </li>
                                        @endif
                                    </ul>
...
<div id="appMountPoint">
        <div lang="es-ES" dir="ltr" class="lang_es">
            <div class="bd dark-background has-uma lang_es" lang="es-ES">
                <div class="pinning-header" style="position:absolute;z-index:999;width:100%;">
                    <div class="pinning-header-container">
                        ...
                        <ul class="tabbed-primary-navigation" role="navigation">
                            <li class="navigation-menu" id="navigationMenu">
                                ...
                                <div class="sub-menu theme-lakira" style="opacity:0;display:none;" id="navOculto">
                                    ...
                                    <ul class="sub-menu-list">
                                        <li class="sub-menu-item"><a href="/home">Inicio</a></li>
                                        <li class="sub-menu-item"><a href="/series">Series</a></li>
                                        <li class="sub-menu-item"><a href="/serie/{{$serie->id}}">{{$serie->title}}</a></li>
                                        <li class="sub-menu-item"><a class="current active" href="/serie/{{$serie->id}}/details">Detalles</a></li>
                                    </ul>
                                </div>
                            </li>
                            ...
                            <li class="navigation-tab">
                                <a href="/serie/{{$serie->id}}">{{$serie->title}}</a>
                            </li>
                            <li class="navigation-tab">
                                <a class="current active" href="/serie/{{$serie->id}}/details">Detalles</a>
                            </li>
                        </ul>
                        <div class="secondary-navigation">
                            <div class="nav-element">
                                <div class="account-menu-item">
                                    ...
                                    <div class="account-drop-down sub-menu theme-lakira" style="opacity:0;display:none;position:absolute;z-index:2;" id="menuOculto">
                ...
                <div class="jawBoneFadeInPlaceContainer">
					<div class="jawBoneContainer slider-hover-trigger-layer" tabindex="0">
						<div class="background">
							<div class="image-rotator jawBoneBackground">
								<span>
									<div class="image-rotator-image " style="background-image: url({{$serie->image}}); z-index: 2; opacity: 1; transition-duration: 750ms; filter:brightness(0.2);"></div>
									<div class="image-rotator-image js-transition-node" style="background-image: url({{$serie->image}}); z-index: 1; opacity: 0; transition-duration: 750ms; filter:brightness(0.2);"></div>
								</span>
							</div>
						...
						<div class="jawBone">
							<h2>
								{{$serie->title}}
								<a tabindex="-1" class="jawbone-title-link">
									<div class="title has-jawbone-nav-transition original" style="transform: translateX(0px); opacity: 1; transition-delay: 250ms; transition-duration: 500ms;">
							...
							<div class="jawBoneCommon">
                                <div class="jawBonePanes offset-for-logo">
                                    <div class="jawBonePane" id="pane-ShowDetails" tabindex="-1" style="opacity: 1; transition-duration: 300ms;">
                                        <div class="ptrack-container">
                                            <div class="ptrack-content" data-ui-tracking-context="%7B%22uuid%22:%22a752624a-448e-4044-ba78-e5bf06a7f266%22,%22appView%22:%22jawboneShowDetails%22%7D" data-tracking-uuid="a752624a-448e-4044-ba78-e5bf06a7f266">
                                                ...
                                                <div class="sliderMask">
                                                    <div class="sliderContent">
                                                        ...
                                                        <div class="detailsItem detailsMedia">
                                                            <ul>
                                                                <li class="listLabel"><span>Dirigida por</span></li>
                                                                <li><a href="/series/dir/{{$director->id}}">{{$director->name}}</a></li>
                                                                <li class="listLabel"><span>Reparto</span></li>
                                                                @foreach($actors as $a)
                                                                   <li> <a href="/series/act/{{$a->id}}">{{$a->name}}</a></li>
                                                                @endforeach
                                                            </ul>
                                                        </div>
                                                        </span>
                                                        ...
                                                        <div>
                                                            <h4 class="listLabel">Géneros</h4>
                                                            <ul>
                                                                @foreach($categories as $cat)
                                                                    <li><a href="/series/cat/{{$cat->id}}">{{$cat->category}}</a></li>
                                                                @endforeach
                                                            </ul>
                                                        </div>
                                                        </div>
                                                        <div class="detailsItem detailsReviews doubleWidth">
                                                        <h4 class="listLabel">Reseñas de los suscriptores</h4>
                                                        <div class="memberReviews">
                                                            <ul class="memberReviews">
                                                                @for($x=0;$x<2;$x++)
                                                                <li>
                                                                    <div class="memberReview">
                                                                    <div class="starbar readOnly" aria-hidden="true">
                                                                    @for($k=1;$k<=$reviews[$x]->stars;$k++)
                                                                        <span class="star default icon-star"></span>
                                                                    @endfor
                                                                    @for($l=1;$l<=5-$reviews[$x]->stars; $l++)
                                                                        <span class="star sb-placeholder icon-star" ></span>
                                                                    @endfor
                                                                    </div>
                                                                    <div class="reviewText">{{$reviews[$x]->description}}</div>
                                                                    </div>
                                                                </li>
                                                                @endfor
                                                            </ul>
                                                            <div id="verResenyas" class="seeAllLink">Ver todas las reseñas ({{$reviews->count()}})</div>
                                                        </div>
                                                        </div>
                                                        <div class="detailsItem detailsMaturity">
                                                        <h4 class="listLabel">Clasificación por edad</h4>
                                                        <span class="maturity-rating">
                                                            <a href="#" class="maturity-number">{{$serie->rating_age}}</a>
                                                            <p class="maturityDescription">No recomendada para menores de {{$serie->rating_age}} años</p>
                                                        </span>
                                                        <div class="video-copyright details-copyright">
                                                            <div></div>
                                                        </div>
                                                        </div>
                                                    </div>
							...
							<ul class="menu" role="tablist">
								<li class="Overview" id="tab-Overview">
									<a href="/serie/{{$serie->id}}" role="link" tabindex="0">Información general</a>
									<span></span>
								</li>
								<li class="Episodes" id="tab-Episodes">
									<a href="/serie/{{$serie->id}}/chapters/1" role="link" tabindex="0">Episodios</a>
									<span></span>
								</li>
								<li class="MoreLikeThis" id="tab-MoreLikeThis">
									<a href="/serie/{{$serie->id}}/similar" role="link" tabindex="0">Similares</a>
									<span></span>
								</li>
								<li class="ShowDetails current" id="tab-ShowDetails">
									<a role="link" tabindex="0">Detalles</a>
									<span></span>
								</li>
							</ul>
						</div>
	...
	<script>
        (function() {
            ...
            $("#verResenyas").click(function(){
                $("#modalResenyas").css('display', 'flex'); 
            });

            $("#btnCerrar").click(function(){
                $("#modalResenyas").css('display', 'none'); 
            });
            ...
            var limpiar = function(){
                ...
                $("#s1").attr('class','star sb-placeholder icon-star')  
                $("#s2").attr('class','star sb-placeholder icon-star')  
                $("#s3").attr('class','star sb-placeholder icon-star')  
                $("#s4").attr('class','star sb-placeholder icon-star')  
                $("#s5").attr('class','star sb-placeholder icon-star')
                $("#reviewId").attr('value', '');
            }

            $("#btnEscribir").click(function(){
                limpiar();
                $("#newResenya").css('display', 'block'); 
                $("#btnEscribir").css('display', 'none'); 
                $("#_method").attr('value', 'POST');
            });

            $("#botonesEditar > a").each(function(){
                $(this).click(function(){
                    limpiar();
                    $("#newResenya").css('display', 'block'); 
                    $("#btnEscribir").css('display', 'none');
                    $("#_method").attr('value', 'PUT');
                    $("#reviewId").attr('value', $(this).attr('id'));
                    var review = "#"+"review"+$(this).attr('id');
                    document.getElementById("stars").value= $(review).attr('stars');
                    document.getElementById("stars").innerHTML= $(review).attr('stars');
                    document.getElementById("reviewText").value= $(review).attr('desc');
                    document.getElementById("reviewText").innerHTML= $(review).attr('desc');
                    
                    var stars = "#s" + $(review).attr('stars');
                    $(stars).trigger('click');
                });
            })
            ...
            $("#btnCancelar").click(function(event){
                ...
                $("#newResenya").css('display', 'none'); 
                $("#btnEscribir").css('display', 'unset'); 
                limpiar();
            });

            $("#s1").click(function(){
                document.getElementById("stars").value=1;
                document.getElementById("stars").innerHTML=1;
                $("#s1").attr('class','star hover icon-star')  
                $("#s2").attr('class','star sb-placeholder icon-star')  
                $("#s3").attr('class','star sb-placeholder icon-star')  
                $("#s4").attr('class','star sb-placeholder icon-star')  
                $("#s5").attr('class','star sb-placeholder icon-star')
            ...
            $("#s2").click(function(){
                ...
                document.getElementById("stars").innerHTML=2;
                $("#s1").attr('class','star hover icon-star')  
                $("#s2").attr('class','star hover icon-star')  
                $("#s3").attr('class','star sb-placeholder icon-star')  
                $("#s4").attr('class','star sb-placeholder icon-star')  
                $("#s5").attr('class','star sb-placeholder icon-star')  
            });
            $("#s3").click(function(){
                ...
                document.getElementById("stars").innerHTML=3;
                $("#s1").attr('class','star hover icon-star')  
                $("#s2").attr('class','star hover icon-star')  
                $("#s3").attr('class','star hover icon-star')  
                $("#s4").attr('class','star sb-placeholder icon-star')  
                $("#s5").attr('class','star sb-placeholder icon-star')  
            });
            $("#s4").click(function(){
                document.getElementById("stars").value=4;
                document.getElementById("stars").innerHTML=4;
                $("#s1").attr('class','star hover icon-star')  
                $("#s2").attr('class','star hover icon-star')  
                $("#s3").attr('class','star hover icon-star')  
                $("#s4").attr('class','star hover icon-star')  
                $("#s5").attr('class','star sb-placeholder icon-star')  
            });
            $("#s5").click(function(){
                document.getElementById("stars").value=5;
                document.getElementById("stars").innerHTML=5;
                $("#s1").attr('class','star hover icon-star')  
                $("#s2").attr('class','star hover icon-star')  
                $("#s3").attr('class','star hover icon-star')  
                $("#s4").attr('class','star hover icon-star')  
                $("#s5").attr('class','star hover icon-star')  
            });
        })();
    </script>
